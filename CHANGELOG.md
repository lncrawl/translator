...
- Embedded auth (`create_app(auth=True)`) now also declares an HTTPBasic
  scheme alongside HTTPBearer, so the docs' Authorize dialog offers both —
  matching hosts that accept either credential.
